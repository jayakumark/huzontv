	...
	public JSONObject call() {
		...
		try
		{
			...
			else if(station_object.getAlertMode().equals("test") || station_object.getAlertMode().equals("live"))
			{
				...
				{
					(new Platform()).addMessageToLog("Skipped FB action (post or email) for " + reporter.getDesignation() + ". Tried to set lock but station.lock() returned false.  user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode());
				}
			} // end else (meaning mode is live or test)
		}
