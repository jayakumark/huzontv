	...
	public JSONObject call() {
		...
		try
		{
			...
			else if(station_object.getAlertMode().equals("test") || station_object.getAlertMode().equals("live"))
			{
				...
				boolean successfullylocked = station_object.lock(uuid, "twitter", "individual");
				...
				{	
					...
					station_object.unlock(uuid, "twitter", "individual");		
				}
				else
				{
					twitter_successful = false;
					twitter_failure_message = "Failed to set TW lock for this station. It seems to be in use already.";
					(new Platform()).addMessageToLog("Skipped TW action (post or email) for " + reporter.getDesignation() + ". Tried to set lock but station.lock() returned false.  user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode());
				}
