...
public class SocialUploaderCallable implements Callable<JSONObject> {
	...
	public JSONObject call() {
		...
		try
		{
			(new Platform()).addMessageToLog("" + social_abbrev + " triggered for " + reporter.getDesignation() + " mode=" + station_object.getAlertMode());
			if(station_object.getAlertMode().equals("silent"))
			{
				social_successful = false;
				social_failure_message = "Station alert_mode is set to silent";
				(new Platform()).addMessageToLog("" + social_abbrev + " triggered but suppressed for " + reporter.getDesignation() + ". mode=silent");
			}
			else if(!reporter.isSocialActive(social_type))
			{
				social_successful = false;
			...
			else if(station_object.getAlertMode().equals("test") || station_object.getAlertMode().equals("live"))
			{
				...
				{	
					if(postinguser.getTwitterAccessToken() == null || postinguser.getTwitterAccessToken().equals("") || postinguser.getTwitterAccessTokenSecret() == null || postinguser.getTwitterAccessTokenSecret().equals(""))
					{
						...
						(new Platform()).addMessageToLog("" + social_abbrev + " triggered for " + reporter.getDesignation() + " but failed due to lack of tw credentials. user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode());
					...
					else // postinguser appears to have social credentials
					{
						...
						if(image_urls == null)
						{
							...
							(new Platform()).addMessageToLog("" + social_abbrev + " triggered for " + reporter.getDesignation() + " with cred. but 2x2 failed. user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode());
						}
						else
						{
							...
							try 
							{
								...
								{	
									...
									if(twit_jo.has("response_status") && twit_jo.getString("response_status").equals("error")) // if an error was produced
									{
										...
										if(twit_jo.has("twitter_code") && (twit_jo.getInt("twitter_code") == 32 || twit_jo.getInt("twitter_code") == 89)) // and it was due to bad credentials
										{
											...
											{
												(new Platform()).addMessageToLog("test account " + social_abbrev + " creds invalid trying to fire for " + reporter.getDesignation() + ". user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode());
											}
										}
									...
									else
									{
										...
										(new Platform()).addMessageToLog("" + social_abbrev + " successful for " + reporter.getDesignation() + " user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode());
										...
										if(alert_text_update_successful && social_id_update_successful)
											se.sendMail("" + social_abbrev + " successful for " + reporter.getDesignation(), "Text and social id updated correctly, too. user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode() + "\n\nhttps://www.huzon.tv/controlpanel.html", "cyrus7580@gmail.com", "info@huzon.tv");
										else
											se.sendMail("" + social_abbrev + " (mostly) successful for " + reporter.getDesignation(), "Text and social id did not update correctly in the DB, though, though. user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode() + "\n\nhttps://www.huzon.tv/controlpanel.html", "cyrus7580@gmail.com", "info@huzon.tv");
									}
								}
								else if(social_type.equals("facebook"))
								{
								...
								(new Platform()).addMessageToLog("IOException trying to create composite image and post to " + social_type + " for " + reporter.getDesignation() + ". " + e.getMessage() + " user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode());
							}
						}
					}
				...
				{
					social_successful = false;
					social_failure_message = "Failed to set " + social_abbrev + " lock for this station. It seems to be in use already.";
					(new Platform()).addMessageToLog("Skipped " + social_abbrev + " action (post or email) for " + reporter.getDesignation() + ". Tried to set lock but station.lock() returned false.  user=" + postinguser.getDesignation() + ". mode=" + station_object.getAlertMode());
				}
